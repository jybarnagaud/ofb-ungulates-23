...
load("data/pour_jy_will_1ha.Rdata") 
abr <- data
...
## on récupère les coefficients et leur significativé
val <- vector("list" , 11)
count <- 1
for(i in qui) {
    plot_data <- ggplot_build(get(i))$data[[3]]
    param <- data.frame(
        term = plot_data$x,
        estimate = plot_data$y,
        conf.low = plot_data$ymin,
        conf.high = plot_data$ymax
    )
    param <- param[order(param$term , decreasing = TRUE),]
    param$pval <- tmp <- apply(param  , 1 , function(x) {
        length(unique(c(x[["conf.low"]]>0, x[["conf.high"]]>0)))
    })
    param$pval[param$pval==1] <- 0.001
    param$pval[param$pval==2] <- 0.99
...
nm <- NULL
for(i in qui) {
    nm <- append(nm , get(i)$labels$title)
}



## les betas
bet <- do.call(cbind, lapply(val, function(x) x[, 2]))
colnames(bet) <- nm
rownames(bet) <- c("log(Species richness)",
                    "Appetency",
                    "Years",
                    "Elevation",
                    "log(Shots)",
...
## la significance
sig <- do.call(cbind, lapply(val, function(x) x[, 5]))
colnames(sig) <- nm
rownames(sig) <- c("log(Species richness)",
                    "Appetency",
                    "Years",
                    "Elevation",
                    "log(Shots)",
...
p <- ggcorrplot(t(bet),
           ...
           insig =  "blank",
           outline.col = "white",
#           title = "Coefficients from Linear Model",
           legend.title = "Coefficient")

p + scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, limit = range(bet)*1.01, name = "Coefficient")
